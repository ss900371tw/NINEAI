...
from	google.oauth2	import	service_account	#	確保最上方有導入這行

def	perform_google_ocr(pdf_file_bytes):
				"""使用	Google	Cloud	Vision	API	進行辨識	(讀取	Streamlit	Secrets)"""
				
				#	檢查	Secrets	是否存在
				if	"gcp_service_account"	not	in	st.secrets:
								st.error("❌	找不到	Secrets	設定：請在	Streamlit	Cloud	中加入	[gcp_service_account]")
								return	""
